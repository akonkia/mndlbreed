...
#' @export
allelesSim <- data.frame(c("A", "a", "A", "a", "A", "a"), c("H", "h", "H", "h","H", "h"), c("W", "w", "W", "w", "W", "w"), c("T", "t", "T", "t", "T", "t"),c("S", "s", "S", "s","S", "s"), c("M", "m", "M", "m","M", "m"),c("F", "f", "F", "f", "F", "f"))
#' @export
alleles <- allelesSim
...
#' @export
allelesCod <- data.frame(c("A","a", "@", "A","a", "@"), c("H", "h", "H", "h", "H", "h"), c("W", "w", "W", "w", "W", "w"), c("T", "t", "T", "t", "T", "t"),c("S", "s", "S", "s", "S", "s"), c("M", "m", "M", "m", "M", "m"),c("F", "f", "F", "f", "F", "f"))
...
getGenotype <- function(dragon){
  gen <- unlist(dragon)
  sorted <- sort(gen)
  order <- c("A", "@", "a", "F", "f", "H", "h", "M", "m", "S", "s", "T", "t", "W", "w")
  sorted <- sorted[order(match(sorted,order))]
  return(sorted)
}
...
#################Plot genotype#############
#' @export
#' If you add codominance, remember to add TRUE to the call.
plotGenotype <- function(dragon, name = "Dragon", codominance = TRUE){
  geno <- getGenotype(dragon)
  genesC <- addCodominance(codominance)[[1]]
  df <- genesC[genesC$`Allele 1`%in% geno | genesC$`Allele 2` %in% geno | genesC$`Allele 3` %in% geno, ] 
  df[,1:8]
  ...
  #text2 <- text_grob("Test", family = "mono", size = 16)
...
######create phenotype###################################
#####If you have added codominance, remember to change the codominance to TRUE.
#' @export
getPhenotype <- function(dragon, codominance = FALSE){
  phenoTable <- addCodominance(codominance)[[3]]
...
#' @export
phenotypeSward <- function(sward, codominance = FALSE){
  phenoTable <- addCodominance(codominance)[[3]]
  phenotypes <- list()
  for (each in sward){
...
#' Pass TRUE as the second argument, if you want to add codominance.
#' @export
...
#' @export
#' Remember to add codominance = TRUE if you add codominance
shortenPhenoTable <- function(traits, codominance = FALSE){
  phenoTable <- addCodominance(codominance)[[3]]
...
recessive <- c("a", "h", "w", "t", "s", "m", "f")
dominant <- c("A", "@","H", "W", "T", "S", "M", "F")
...
homozygoteRe <- function(dragon){
  ...
  if(dominant[2] %in% dragon$set1 | dominant[2] %in% dragon$set2){
    print("Yes")
    dragon$set1[dominant[2] == dragon$set1] <- "a"
    dragon$set2[dominant[2] == dragon$set2] <- "a"
  }
  return(dragon)
}
#' @export
homozygoteDo <- function(dragon){
  ...
  if(dominant[2] %in% dragon$set1 | dominant[2] %in% dragon$set2){
    pick <- sample(c("A", "@"), 1)
    dragon$set1[dominant[2] == dragon$set1 | dominant[1] == dragon$set1] <- pick
    dragon$set2[dominant[2] == dragon$set2 | dominant[1] == dragon$set2] <- pick
  }
  return(dragon)
...
heterozygote <- function(dragon){
  dragon$set1 <- toupper(dragon$set1)
  if(dominant[2] %in% dragon$set1){
    pick <- sample(c("A", "@"), 1)
    dragon$set1[dominant[2] == dragon$set1 | dominant[1] == dragon$set1] <- pick
  }
  
  dragon$set2 <- tolower(dragon$set2)
  if(dominant[2] %in% dragon$set2){
    dragon$set2[dominant[2] == dragon$set2] <- "a"
  }
  return(dragon)
}